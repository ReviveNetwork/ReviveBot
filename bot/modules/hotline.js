...
bot.on('message', message => {
    ...
    if (message.channel.id == '271350052188979201') {
        bot.channels.get('271349742099759104').send(attach, { embed: m2e(message, true) })
        ...
        bot.channels.get('271350052188979201').send(attach, { embed: m2e(message, true) })
            .then(msg => messageDB.data.push({ oldMessage: message.id, newMessage: msg.id, channel: msg.channel.id }));
    }
});
bot.on('messageUpdate', (oldMessage, newMessage) => {
    ...
    if (m) {
        //console.log(m);
        bot.channels.get(m.channel).fetchMessage(m.newMessage).edit(attach, { files: [attach], embed: m2e(newMessage, true) });;
    }
});
