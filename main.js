...
function startlog() {
    ...
    pm2.stderr.on('data', (data) => {
        ...
        bot.channels.get(config.log_channel).sendCode("shell", 'Log- ' + stderr, { split: true });
    });

    pm2.stdout.on('data', (data) => {
        //console.log(data.toString());
        bot.channels.get(config.log_channel).sendCode("shell", 'ERROR - ' + stderr, { split: true });
    });
